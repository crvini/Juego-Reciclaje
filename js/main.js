...
function iniciarJuegoUnJugador() {
    ...
    function agregarPuntos(tipo) {
        if (tipo === 'recicable') {
            score += 3; // Sumar 3 puntos si es reciclable
        } else {
            score++; // Sumar 1 punto para otros tipos
        }
        document.getElementById('score').textContent = `Dinero Obtenido: Q${score}`;
    }
    ...
    function verificarObjetosColocados() {
        if (placedObjects === totalObjects) {
            ...
            if (score >= 18) {
                ...
                document.getElementById('winner').textContent = "¡Perdiste! Obtuviste menos de Q18";
            }
            modal.style.display = 'block';
        }
    }
